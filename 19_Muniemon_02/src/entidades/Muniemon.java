...
public class Muniemon {
	...
	public void atacar(Muniemon muniemonB) {

		Interfaz.openingFuncion(this.nombre + " ataca a " + muniemonB.nombre);
		...
		if (this.vida == 0) {
			...
			if (this.ataque > muniemonB.defensa) {
				muniemonB.vida -= this.ataque - muniemonB.defensa;
				System.out.println("Le quitaste " + (this.ataque - muniemonB.defensa)
						+ " puntos de vida");
				if (muniemonB.vida < 0) {
					muniemonB.vida = 0;
					System.out.println("Mataste a " + muniemonB.nombre);
				}
			} else if (this.ataque <= muniemonB.defensa) {
